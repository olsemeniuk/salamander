...
function manageImageBackrounds(){
	...
	productImageWrappers.forEach(productImageWrapper => {
		if (productImageWrapper.querySelector('img[src$=".jpg"]') || productImageWrapper.querySelector('img[src$=".jpeg"]')) {
			//console.log('here')
...
function testRegion(){
	...
	let recRegion = 'ru'; //should be getting of reccomendation
...
let hints = {
	...
	'sr': {options: {isImportant: true}, hintSide: 'right', title: 'Проверьте свой ПК', text: 'Не разбираетесь в технических характеристиках? Не беда. Мы сами сверим основные характеристики за вас. Для этого авторизуйтесь и заполните информацию о вашем ПК.', prevHints: ['price-header'],
	...
	'subscribe-error': {hintSide: 'right', title: 'Введите свой email', text: 'Пожалуйста, введите в настройках свой адрес элекстронной почты, чтобы иметь возможность следить за изменением минимальных цен на игры.',
		button: {
			icon: {styles: ['btn-icon-new', 'btn-user-settings', 'static']},
			type: 'a',
			text: 'Настройки',
